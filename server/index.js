...
// ---------------------------------------------------------------
// запросы к api
app.use('/api', require('./api/site/feedback'));
...
http.listen(config.server.port, () => {
    ...
    return Promise.resolve(logUtils.initFileLogger())
        ...
        .then(result => {
            const message = `Hosted on:  ${config.server.host}:${config.server.port}`;

            console.log(message);

            return Promise.resolve(logUtils.fileLogMessage(message));
        })
        .catch(error => {
